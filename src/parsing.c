...
/*   Updated: 2025/04/28 12:52:59 by zamohame         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child_process(t_pipex *pipex, int fd_in, int fd_out, char **cmd)
{
	...
	if (dup2(fd_in, STDIN_FILENO) == -1 || dup2(fd_out, STDOUT_FILENO) == -1)
	{
		...
		close(fd_out);
		clean_up(pipex);
		error_exit("Error: dup2 failed", 1, NULL);
	}
	close(fd_in);
	close(fd_out);
	if (!cmd || !cmd[0])
	{
		clean_up(pipex);
		error_exit("Error: command not found", 127, NULL);
	}
	cmd_name = ft_strdup(cmd[0]);
	if (!cmd_name)
	{
		clean_up(pipex);
		error_exit("Error: memory allocation failed", 1, NULL);
	}
	child_path(pipex, cmd, cmd_name);
}
...
int	execute_commands(t_pipex *pipex)
{
	...
	if (pipe(pipex->fd) == -1)
		error_exit("Error: pipe creation failed", 1, NULL);
	pid1 = fork();
	fork_error(pipex, pid1, 0);
	if (pid1 == 0)
	{
		close(pipex->fd[0]);
